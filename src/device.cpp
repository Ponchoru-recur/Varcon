...
std::vector<const char *> LveDevice::getRequiredExtensions() {
    unsigned sdlExtCount = 0;
    const char *const *extNames = SDL_Vulkan_GetInstanceExtensions(&sdlExtCount);
    if (!extNames) {
        throw std::runtime_error("[ Error ] SDL_Vulkan_GetInstanceExtensions failed\n");
    }

    std::vector<const char *> extensions(extNames, extNames + sdlExtCount);
